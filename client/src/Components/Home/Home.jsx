...
import { getAllDogs, getTemperaments, FilterByTemperament, OrderByName, OrderByWeight, filterByDB } from "../../Redux/actions/index";
        ...
        <>
            ...
            <div className={`${style.main_container}`}>
                        ...
                        return (
                            <div id={`${style.container_dog}`} key={el.id}>
                                <Link to={'/dog-detail/' + el.id}>
                                    {
                                        <Dogs key={el.id} image={el.image} name={el.name} temperaments={el.temperaments[0].name ? el.temperaments.map((el) => el.name) : el.temperaments} /> 
                                    }
                                </Link>
                            </div>
                        )
                ...
                <div className={`${style.container_paginate}`}>
                    <Paginate setCurrentPage={setCurrentPage} dogsByPage={dogsByPage} allDogs={allDogs.length} paginated={paginated} currentPage={currentPage} />
                </div>
            </div>
        </>
    );
